...
namespace BlueprintsEverywhere.Detour
{
	...
	{
		// occupiedCellsTemp was originally private
		internal static List<IntVec3> occupiedCellsTemp = new List<IntVec3>();
...
*/
		internal static bool _CanConstruct(Thing t, Pawn p)
		{
...
*/
		internal static AcceptanceReport _CanPlaceBlueprintAt(BuildableDef entDef, IntVec3 center, Rot4 rot, bool godMode = false)
		{
			_GenConstruct.occupiedCellsTemp.Clear();
			foreach (IntVec3 current in GenAdj.CellsOccupiedBy(center, rot, entDef.Size))
			{
				_GenConstruct.occupiedCellsTemp.Add(current);
			}
			for (int i = 0; i < _GenConstruct.occupiedCellsTemp.Count; i++)
			{
				IntVec3 c = _GenConstruct.occupiedCellsTemp[i];
				if (!c.InBounds())
				{
				...
				foreach (IntVec3 current2 in GenAdj.CellsAdjacentCardinal(center, rot, entDef.Size))
				{
					if (current2.InBounds())
					{
						foreach (Thing current3 in Find.ThingGrid.ThingsAt(current2))
						{
							...
							if (thingDef3.hasInteractionCell && _GenConstruct.occupiedCellsTemp.Contains(Thing.InteractionCellWhenAt(thingDef3, current3.Position, current3.Rotation)))
							{
